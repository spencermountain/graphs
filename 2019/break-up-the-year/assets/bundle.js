...
var compute = function compute(num, year) {
  var parts = [];
  var start = spacetime.now().year(year).startOf('year');
  var end = spacetime.now().year(year).endOf('year');
  var diff = end.epoch - start.epoch;
  var part = diff / num;

  for (var i = 0; i < num; i += 1) {
    var epoch = start.epoch + part * i;
...
},{"spacetime":5}],2:[function(_dereq_,module,exports){
"use strict";
...
var sometime = _dereq_('sometime'); // const colors = require('spencer-color').list


var combos = _dereq_('spencer-color').combos;

var colors = [].concat(_toConsumableArray(combos.ken), _toConsumableArray(combos.dupont), _toConsumableArray(combos.bloor));

var plusMinus = _dereq_('./plus-minus');
...
var makeList = function makeList(parts) {
  var list = parts.map(function (s, i) {
    var style = "border-bottom:3px solid " + colors[i];
    return h(_templateObject(), i + 1, style, s.format('nice'));
  });
  var html = "<div class=\"col grey left\">".concat(list.join(''), "</div>");
  document.querySelector('#datelist').innerHTML = html;
};

var makeCalendar = function makeCalendar(parts, year) {
  var cal = sometime.year.ByQuarter('nov 9th ' + year, {});
  parts.forEach(function (s, i) {
    ...
    var end = parts[i + 1] || s.endOf('year'); // end = end.minus(1, 'day')
...
var doit = function doit() {
  var pieces = Number(document.querySelector('#plusMinus').value);
  var year = Number(document.querySelector('#yearNum').value);
  var parts = compute(pieces, year);
  makeList(parts);
...
},{"./compute":1,"./plus-minus":8,"htm":3,"sometime":4,"spencer-color":6,"vhtml":7}],3:[function(_dereq_,module,exports){
...
function _templateObject() {
  var data = _taggedTemplateLiteral(["<div class=\"row mw10\">\n    <input id=\"", "\" type=\"button\" value=\"-\"/>\n    <input class=\"w5 f2 grey rounded center\" id=\"", "\" type=\"text\" value=\"", "\"/>\n    <input id=\"", "\" type=\"button\" value=\"+\"/>\n  </div>"]);
...
var build = function build(id) {
  var val = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 0;
  id = id || 'plusMinus';
  return h(_templateObject(), 'minus' + id, id, val, 'plus' + id);
};

var events = function events(id, cb) {
  id = id || 'plusMinus';
