...
var calendar = _dereq_('somehow-calendar');

var schedules = _dereq_('./data/schedules');

var input = _dereq_('somehow-input');
...
var drawTeam = function drawTeam(team) {
  ...
  schedule.forEach(function (game) {
    var date = game.date + ' 2019';
    var color = game.home ? home : away; // cal.underline(date, color)
...
document.querySelector('#select').innerHTML = input.select({
  options: schedules.map(function (o) {
    return o.team;
  }),
  cb: function cb(val) {
    drawTeam(val);
