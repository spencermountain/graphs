...
},{"../../parse":24}],39:[function(_dereq_,module,exports){
"use strict";

// const somehow = require('somehow');
var somehow = _dereq_('/Users/spencer/mountain/somehow/src');

var data = _dereq_('./data/toronto-montreal');

var w = somehow({
  height: 80,
  width: 700 // aspect: 'widescreen',

});
var canada = data[2].years.map(function (a) {
  a[0] = 'Jan 1 ' + String(a[0]);
  return a;
});
var can = w.area().color('lightgrey').opacity(0.3);
can.set(canada);
w.text('36m').color('red').font(12).center('102%', 36000000).dy(-10); //ww1

w.line().from('Jul 28, 1914', '0px').to('Nov 11, 1918', '0px').color('red');
w.text('ww1').center('Jul 28, 1915', '-15px').dx(5).font(14).color('red'); //ww2

w.line().from('Sept 1 1939', '0px').to('Sept 2 1945', '0px').color('red');
w.text('ww2').center('Sept 1 1941', '-15px').dx(5).font(14).color('red');
w.text('Canada:').color('red').center('62%', '50px'); //confederation
...
w.yAxis.remove();
w.fit();
var el = document.querySelector('#canada');
console.log(el);
el.innerHTML = w.build();

},{"./data/toronto-montreal":43,"/Users/spencer/mountain/somehow/src":17}],40:[function(_dereq_,module,exports){
"use strict";

var somehow = _dereq_('somehow');

var data = _dereq_('./data/winning-years');

var w = somehow({
  height: 110,
  width: 700
});
data.toronto.forEach(function (year) {
  w.line().color('blue').set([["jan 1 ".concat(year), 3], ["sept 1 ".concat(year), 3]]);
});
data.montreal.forEach(function (year) {
  w.line().color('red').set([["jan 1 ".concat(year), 2], ["sept 1 ".concat(year), 2]]);
});
...
w.y.fit(4, 0);
w.yAxis.remove();
var el = document.querySelector('#hockey-timeline');
el.innerHTML = w.build();

},{"./data/winning-years":44,"somehow":46}],41:[function(_dereq_,module,exports){
"use strict";
...
var toronto = data[0].years.map(function (a) {
  a[0] = 'Jan 1 ' + String(a[0]);
  return a;
});
var tor = w.line().color('blue');
tor.set(toronto);
var lastTo = toronto[toronto.length - 1];
w.text('6.3m').dx(5).font(12).color('blue').at(lastTo[0], lastTo[1]);
var montreal = data[1].years.map(function (a) {
  a[0] = 'Jan 1 ' + String(a[0]);
  return a;
});
var mon = w.line().color('orange');
mon.set(montreal);
var lastMtl = montreal[montreal.length - 1];
w.text('4.1m').dx(5).font(12).color('orange').at(lastMtl[0], lastMtl[1]); // w.yAxis.ticks([1000000, 2000000]);

w.yAxis.ticks([]);
w.text(['population']).color('lightgrey').font(10).from('-55px', '75%'); //referendums

w.line().from('May 20, 1980', '100px').to('May 20, 1980', '140px').width(1).color('grey');
w.text(['1st', 'referendum']).dy(-35).center('May 20, 1980', '100px').font(11).color('lightgrey');
w.line().from('October 30, 1995', '90px').to('October 30, 1995', '130px').width(1).color('grey');
w.text(['2nd', 'referendum']).dy(-35).center('October 30, 1995', '90px').font(11).color('lightgrey');
w.fit();
w.x.fit('Jan 1 1825', Date.now());
var el = document.querySelector('#stage');
el.innerHTML = w.build();

},{"./data/toronto-montreal":43,"/Users/spencer/mountain/somehow/src":17}],42:[function(_dereq_,module,exports){
"use strict";
...
var makeTimeline = function makeTimeline(year) {
  var w = somehow({
    height: 10,
    width: 700
  });
  var date = "Jan 1 ".concat(year);
  w.x.fit('Jan 1 1825', Date.now());
  w.y.fit(0, 3);
  w.yAxis.remove();
  var ticks = [{
...
},{"somehow":46}],43:[function(_dereq_,module,exports){
"use strict";

module.exports = [{
  "title": "Toronto",
  "years": [[1825, 6000 //i made this number up
  ], [1834, 9252], [1901, 238080], [1931, 856955], [1941, 951549], [1951, 1176622], [1961, 1824481], [1971, 2089729], [1976, 2124291], [1981, 2137395], [1986, 2192721], [1991, 2275771], [1996, 2385421], [2001, 2481494], [2006, 2503281], [2011, 2615060], [2016, 2731571]]
}, {
  "title": "Montreal",
  "years": [// [
  //   1740,
  //   22000
  // ],
  [1825, 26154], [1871, 141276], [1881, 189168], [1891, 271352], [1901, 347817], [1911, 533341], [1921, 693225], [1931, 959198], [1941, 1064653], [1951, 1247647], [1956, 1402704], [1961, 1607601], [1966, 1750969], [1971, 1765553], [1976, 1664527], [1981, 1554761], [1986, 1541251], [1991, 1553356], [1996, 1550369], [2001, 1583590], [2006, 1620639], [2011, 1649519], [2016, 1704694]]
}, {
  "title": "Demographics of Canada",
  "years": [[1825, 30000 //i made this number up
  ], [1851, 2414519], [1861, 3174442], [1871, 3689000], [1881, 4325000], [1891, 4833000], [1901, 5371000], [1911, 7207000], [1921, 8788000], [1931, 10377000], [1941, 11507000], [1951, 14050000], [1961, 18271000], [1971, 21962032], [1981, 24819915], [1991, 28037420], [2001, 31020596], [2011, 33476688], [2016, 35151728]]
}];
...
module.exports = {
  "toronto": [// 1944,
  1946, 1947, 1948, 1950, 1961, 1962, 1963, 1966],
  "montreal": [// 1943,
  // 1945,
  1952, 1955, 1956, 1957, 1958, 1959, 1964, 1965, 1967, 1968, 1970, 1972, 1975, 1976, 1977, 1978, 1985, 1992]
};
...
},{"./_canada":39,"./_hockey":40,"./_population":41,"./_timelines":42}],46:[function(_dereq_,module,exports){
(function (global){
/* somehow v0.0.8
