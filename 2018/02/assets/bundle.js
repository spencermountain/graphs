...
var drawCity = function drawCity(w, city) {
  var arr = city.weather.split(',').map(function (temp, i) {
    var date = months[i] + ' 1 ' + year;
    return [date, Number(temp)];
  }); //add start to very end

  arr.push(["Dec 31 ".concat(year), arr[0][1]]);
...
},{"./_drawCity":1,"./data/by-latitude":3,"./data/labels":4}],3:[function(_dereq_,module,exports){
"use strict";
...
w.line().color('lighter').width(1).set([[today, '20%'], [today, '80%']]); // 0 mid-line

w.line().at(null, 0).color('lightgrey').width(1).dotted(true); //make initial line to share around
...
w.y.fit(-40, 40);
w.x.fit("Jan 1 ".concat(year), "Dec 31 ".concat(year));
w.yAxis.ticks([{
  value: -30,
  label: '-30°'
}, {
...
},{"./_drawCity":1,"./_slider":2,"./data/by-latitude":3,"somehow":6}],6:[function(_dereq_,module,exports){
(function (global){
/* somehow v0.0.8
