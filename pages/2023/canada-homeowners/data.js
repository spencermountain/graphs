...
const data = [
  ...
  { rate: 0.7, start: 45, label: '45 to 49', total: 2359965 }, //	6.7%
...
// cumulative % of population at-or-above each age group
let run = 0
...
// houses owned by each age group
for (let i = 0; i < data.length; i += 1) {
  data[i].houses = data[i].total * data[i].rate
}
let houses = data.reduce((h, n) => h + n.houses, 0)

// cumulative % of houses owned at-or-above each age group
run = 0
for (let i = data.length - 1; i >= 0; i -= 1) {
  run += data[i].houses
