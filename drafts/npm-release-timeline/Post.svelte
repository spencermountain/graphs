<script>
  ...
  releases.then(a => {
    ...
    start = start.clone().minus(3, 'weeks')
  })
  text.subscribe(str => {
    console.log(str)
  })
  ...
  <div class="m3 container">
    ...
    {:then releases}
      ...
      <Timeline {start} {end} {height}>
        ...
        <Column width="100px" label="Patch" color="blue">
          {#each arr as release}
            {#if release.type === 'patch'}
              <Dash start={release.date} color="blue" opacity="0.5" dotted={false} />
            {/if}
          {/each}
        </Column>
