...
module.exports = [[2019, 7714576923], [2018, 7632819325], [2017, 7550262101], [2016, 7466964280], [2015, 7383008820], [2014, 7298453033], [2013, 7213426452], [2012, 7128176935], [2011, 7043008586], [2010, 6958169159], [2009, 6873741054], [2008, 6789771253], [2007, 6706418593], [2006, 6623847913], [2005, 6542159383], [2004, 6461370865], [2003, 6381408987], [2002, 6302149639], [2001, 6223412158], [2000, 6145006989], [1999, 6066867391], [1998, 5988846103], [1997, 5910566295], [1996, 5831565020], [1995, 5751474416], [1994, 5670319703], [1993, 5588094837], [1992, 5504401149], [1991, 5418758803], [1990, 5330943460], [1989, 5240735117], [1988, 5148556956], [1987, 5055636132], [1986, 4963633228], [1985, 4873781796], [1984, 4786483862], [1983, 4701530843], [1982, 4618776168], [1981, 4537845777], [1980, 4458411534], [1979, 4380585755], [1978, 4304377112], [1977, 4229201257], [1976, 4154287594], [1975, 4079087198], [1974, 4003448151], [1973, 3927538695], [1972, 3851545181], [1971, 3775790900], [1970, 3700577650], [1969, 3625905514], [1968, 3551880700], [1967, 3479053821], [1966, 3408121405], [1965, 3339592688], [1964, 3273670772], [1963, 3210271352], [1962, 3149244245], [1961, 3090305279], [1960, 3033212527], [1959, 2977824686], [1958, 2924081243], [1957, 2871952278], [1956, 2821383444], [1955, 2772242535], [1954, 2724302468], [1953, 2677230358], [1952, 2630584384], [1951, 2583816786], [1927, 2000000000], [1910, 1750000000], [1900, 1600000000], [1850, 1200000000], [1804, 1000000000], [1760, 770000000], [1700, 610000000], [1600, 500000000], [1500, 450000000], [1400, 350000000], [1200, 360000000], [1100, 320000000], [1000, 275000000], [900, 240000000], [800, 220000000], [700, 210000000], [600, 200000000], [200, 190000000], [-200, 150000000], [-500, 100000000], [-1000, 50000000], [-2000, 27000000], [-3000, 14000000], [-4000, 7000000], [-5000, 5000000]];
...
var drawIt = function drawIt(gens, age, genLength, gender) {
  ...
  for (var i = 0; i < gens; i += 1) {
    ...
    w.text(name).at(d.iso(), i).font(10).dy(2).dx(5);
  ...
  showEvents(w);
  w.yAxis.remove();
  document.querySelector('#graph').innerHTML = w.build(); //re-draw earth population graph too
...
},{"./events":6,"./world":8,"somehow":3,"spacetime":4}],6:[function(_dereq_,module,exports){
"use strict";

//
var dy = '-30px';
var events = [['Dec 5 1492', ['columbus', 'landing']]];

var showEvents = function showEvents(w) {
  //ww2
  w.rect().color('red').set([['Sep 1 1939', '0%'], ['Sep 2 1945', '100%']]).rounded(1);
  w.text('WWII').center('Sep 1 1941', dy).font(12); //ww1

  w.rect().color('red').set([['Jul 28 1914', '0%'], ['Nov 11 1918', '100%']]).rounded(1);
  w.text('WWI').center('Sep 1 1916', dy).font(12); //centuries

  var centuries = [2000, 1900, 1800, 1700];
  centuries.forEach(function (c) {
    w.line().set([["Jan 1 ".concat(c), '0%'], ["Jan 1 ".concat(c), '100%']]).dotted(true).width(1).color('grey');
  }); //other events

  events.forEach(function (event) {
    w.line().set([[event[0], '0%'], [event[0], '100%']]).dotted(true).width(1).color('sky');
...
var genderSelect = inputs.select({
  width: 800,
  max: 75,
  min: 1,
  value: 'paternal',
...
},{"./draw":5,"somehow-input":2}],8:[function(_dereq_,module,exports){
"use strict";
...
},{"../data/world-population":1,"somehow":3,"spacetime":4}]},{},[7]);
